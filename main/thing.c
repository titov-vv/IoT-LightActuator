...
{
    ESP_LOGI(TAG_AWS, "Delta callback");
    ESP_LOGI(TAG_AWS, "%.*s\t%.*s", topicNameLen, topicName, (int) params->payloadLen, (char *)params->payload);
}
//-----------------------------------------------------------------------------
static void status_callback(AWS_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
                                    IoT_Publish_Message_Params *params, void *pData)
{
    ESP_LOGI(TAG_AWS, "Status callback");
    ESP_LOGI(TAG_AWS, "%.*s\t%.*s", topicNameLen, topicName, (int) params->payloadLen, (char *)params->payload);
}
//-----------------------------------------------------------------------------
...
static IoT_Error_t subscribe_topics(AWS_IoT_Client *client)
{
	...
	res = aws_iot_mqtt_subscribe(client, delta_topic, strlen(delta_topic), QOS0, delta_callback, NULL);
	if (res != SUCCESS)
	{
	...
	res = aws_iot_mqtt_subscribe(client, accepted_topic, strlen(accepted_topic), QOS0, status_callback, NULL);
	if (res != SUCCESS)
	{
	...
	res = aws_iot_mqtt_subscribe(client, rejected_topic, strlen(rejected_topic), QOS0, status_callback, NULL);
	if (res!= SUCCESS)
	{
...
{
	IoT_Error_t res = FAILURE;
	char update_topic[] = "$aws/things/" AWS_CLIENTID "/shadow/update";
	...
	while (1)
	{
		...
		case NETWORK_RECONNECT_TIMED_OUT_ERROR:
			...
			else
			{
				...
				{
					retry_cnt = 0;
					vTaskDelay(3600000*retry_cnt / portTICK_RATE_MS);
				}
			}
			continue;
			break;
